...
void render2DTree(Node *node, pcl::visualization::PCLVisualizer::Ptr &viewer, Box window, int &iteration, uint depth = 0)
{
	if (node != NULL)
	{
		...
		if (depth % 2 == 0)
		{
			std::cout << "Rendering split on x axis" << std::endl;
		...
		{
			std::cout << "Rendering split on y axis" << std::endl;
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	for (auto idx = 0; idx < points.size(); idx++)
	{
		if (!processed[idx])
		{
			std::cout << "Querying point " << idx << std::endl;
			processed[idx] = true;

			// Create cluster + mark as processed
			auto cluster = tree->search(points[idx], distanceTol);
			cout << "Found cluster with indexes: ";
